  ...
  const TabButton = ({ id, label, icon: Icon, count }: { id: "experiences" | "skills", label: string, icon: any, count?: number }) => (
    <button
      ...
      className={`flex-1 flex items-center justify-center gap-3 px-6 py-4 border-b-2 transition-all duration-300 group ${
        activeTab === id 
          ? "border-zinc-500 bg-zinc-500/5 text-white" 
          : "border-transparent text-gray-500 hover:text-gray-300 hover:bg-white/5"
      }`}
    >
      <Icon className={`w-5 h-5 transition-colors ${activeTab === id ? "text-zinc-400" : "text-gray-600 group-hover:text-gray-400"}`} />
      ...
      {count !== undefined && (
        <span className={`ml-2 px-1.5 py-0.5 rounded text-[10px] font-mono ${activeTab === id ? "bg-zinc-500/20 text-zinc-400" : "bg-white/5 text-gray-600"}`}>
          {count}
        </span>
      )}
    </button>
