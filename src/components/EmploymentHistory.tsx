...
const EmploymentHistory: React.FC = () => {
  const [activeTab, setActiveTab] = useState<"professional" | "research">(
    "professional",
  );
  ...
  const filteredJobs = employmentHistoryData.filter(
    (job) => job.type === activeTab,
  );
  ...
  const TabButton = ({
    ...
    >
      <Icon
        className={`w-5 h-5 transition-colors ${activeTab === id ? "text-blue-400" : "text-gray-600 group-hover:text-gray-400"}`}
      />
      ...
      <span
        className={`ml-2 px-1.5 py-0.5 rounded text-[10px] font-mono ${activeTab === id ? "bg-blue-500/20 text-blue-400" : "bg-white/5 text-gray-600"}`}
      >
        {count}
      </span>
    </button>
        ...
        {filteredJobs.map((job, index) => {
          ...
          return (
            <div key={index} className="relative pl-10 group">
              {/* Trace Span Icon */}
              <div
                className={`absolute -left-[13px] top-0 w-6 h-6 rounded-full flex items-center justify-center transition-all duration-500 z-10 ${
                  isCurrent
              ...
              <div className="hidden md:block absolute -left-44 top-0 w-36 text-right space-y-1">
                <div
                  className={`text-xs font-mono font-bold tracking-tighter ${isCurrent ? "text-blue-400" : "text-gray-500"}`}
                >
                  {job.dateRange}
                </div>
              ...
              <div
                ...
                className={`glass-panel p-6 cursor-pointer hover:bg-white/[0.08] hover:-translate-y-1 hover:shadow-2xl transition-all duration-300 border-l-4 ${
                  isCurrent
                    ? "border-l-blue-500"
                    : "border-l-white/10 group-hover:border-l-blue-500/30"
                }`}
              >
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                  <div>
                    <h3
                      className={`text-xl font-bold tracking-tight ${isCurrent ? "text-white" : "text-gray-300"}`}
                    >
                      {job.position}
                    </h3>
                  ...
                  <div className="bg-white/5 border border-white/10 px-3 py-1.5 rounded-lg shrink-0">
                    ...
                    <div className="text-[11px] font-bold text-gray-300">
                      {job.company === "Amazon Web Services"
                        ? "AWS Lambda"
                        : job.company}
                    </div>
                  </div>
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {job.technologies.slice(0, 5).map((tech) => (
                    <span
                      key={tech}
                      className="flex items-center gap-1.5 px-2 py-0.5 bg-white/5 border border-white/5 rounded text-[10px] font-mono text-gray-400"
                    >
            ...
            <div className="bg-[#1A1A2E] px-4 py-3 border-b border-white/10 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div className="text-xs font-mono text-gray-400 flex items-center gap-2">
                  ...
                  <span>
                    {selectedJob.company.toLowerCase().replace(/\s/g, "_")}.log
                  </span>
                </div>
              </div>
              <button
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedJob.technologies.map((tech) => (
                      <span
                        key={tech}
                        className="px-3 py-1 bg-blue-500/10 border border-blue-500/20 rounded text-xs font-mono text-blue-300"
                      >
                        {tech}
                      </span>
                    ))}
